...
class TestImageCompositor(unittest.TestCase):
	...
	def test_set_background_and_render_and_versioning(self):
		...
		v1 = comp.set_layer_background(DisplayImage(datetime.datetime.now(), "Background", bg))
		self.assertIsNotNone(v1)
		self.assertIsInstance(v1, LayerStack)
		self.assertEqual(v1.version, 0)
		self.assertEqual(comp.current_version, 1)

		out = comp.render()
		self.assertIsNotNone(out)
		if out is not None:
			did, the_info = out.render()
			self.assertTrue(did)
			...
			if the_info is not None:
				...
				if out3 is not None:
					did3, the_info3 = out3.render()
					self.assertTrue(did3)
