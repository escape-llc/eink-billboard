	...
	def _display_message(self, msg: DisplayImage):
		try:
			...
			if display_settings is not None:
				image = change_orientation(image, display_settings.get("orientation", "landscape"))
				image = resize_image(image, self.resolution)
#				if display_settings.get("rotate180", False): image = image.rotate(180)
#				image = apply_image_enhancement(image, display_settings)

			self.compsitor.set_layer_background(image)
			# TODO set commit timer for final render
